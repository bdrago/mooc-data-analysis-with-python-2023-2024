...
from tmc.utils import get_stdout, load, patch_helper
...
class Subplots(unittest.TestCase):
    def test_calls(self):
        ...
        with patch(ph("plt.show")) as pshow, patch(
            ph("plt.subplots"), side_effect=plt.subplots
        ) as psubplots, patch(ph("plt.subplot"), side_effect=plt.subplot) as psubplot:
            subfigures(a)
            ...
            if psubplot.call_count > 0:
                self.assertEqual(
                    psubplot.call_count,
                    2,
                    "expected plt.subplot to have been called twise",
                )
                ...
                if "nrows" in psubplots.call_args[1]:
                    ...
                    self.assertEqual(
                        psubplots.call_args[0][:2],
                        (1, 2),
                        msg="Wrong shape of subplot grid",
                    )
    ...
    def test_method_calls(self):
        ...
        mysub = MockSubplots()
        with patch(ph("plt.show")) as pshow, patch(
            ph("plt.subplots"), side_effect=mysub
        ) as psubplots, patch(ph("plt.plot"), side_effect=plt.plot) as pplot, patch(
            ph("plt.scatter"), side_effect=plt.scatter
        ) as pscatter:
            subfigures(a)
            pshow.assert_called_once()
            ...
            if len(positional) >= 2:
                ...
                self.assertTrue(
                    False,
                    msg="Give x and y for scatter call as position "
                    "either as positional arguments or as keyword arguments!",
                )

            self.assertIn(
                "c", kwargs, msg="Give the 'c' keyword argument to scatter method call!"
            )
            np.testing.assert_array_equal(
                kwargs["c"],
                a[:, 2],
            ...
            self.assertIn(
                "s", kwargs, msg="Give the 's' keyword argument to scatter method call!"
            )
            np.testing.assert_array_equal(
                kwargs["s"],
                a[:, 3],
